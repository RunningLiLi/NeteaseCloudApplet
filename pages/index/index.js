...
.then(res=>{
    const totalStr=res.list.slice(0,4).reduce((totalStr,item)=>{
        const {updateFrequency,coverImgUrl,tracks}=item;
        const lis=tracks.reduce((lisStr,liItem,key)=>`${lisStr}<li>${key+1}.${liItem.first}-${liItem.second}</li>`,'')
        return `${totalStr}<div class="topList-item"><span class="topList-update">${updateFrequency}</span><img src=${coverImgUrl}><ol>${lis}</ol></div>`
    },'')
